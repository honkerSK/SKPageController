...
@interface SKPageController ()<UIScrollViewDelegate>
{
    UIColor *_norColor;
    UIColor *_selColor;
...
/** 滚动条 */
@property (nonatomic, strong) SKPageProgressView *pregressView;
...
@implementation SKPageController
...
- (void)viewDidLayoutSubviews
{
    ...
    CGFloat tY = (_topDistance != 0) ? _topDistance : (self.navigationController.navigationBarHidden == NO) ? SKNormalTitleViewH + statusH : statusH;
    CGFloat tH = (_titleViewHeight != 0) ? _titleViewHeight : SKNormalTitleViewH;
...
- (void)setUpAllTitle
{
    ...
    CGFloat progressH = (_progressHeight && _progressHeight < SKPageMargin && _progressHeight > 0) ?  _progressHeight : SKUnderLineH;
    ...
    for (NSInteger i = 0; i < VCCount; i++) {
        ...
        _titleButton.titleLabel.font = (!_titleFont) ? SKTitleNorFont : _titleFont;
        _titleButton.tag = i + SKButtonTagValue; //绑定tag(增加额外值)
        buttonX = i * buttonW;
    ...
    if (_isShowPregressView) { //如果没子控制器的时候不加载
        //指示条
        _pregressView = [SKPageProgressView new];
...
- (void)AddOneVcWithButtton:(NSInteger)i
{
    ...
    vc.view.frame = CGRectMake(i * ScreenW, 0,ScreenW , _contentScrollView.sk_height);
...
- (void)titleButtonClick:(UIButton *)button
{
    ...
    NSInteger buttonTag = button.tag - SKButtonTagValue;
